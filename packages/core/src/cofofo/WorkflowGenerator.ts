...
const PHASE_INSTRUCTIONS: Record<Phase, string> = {
  ...
  requirement: 'Read the current CONTEXT-MANIFEST.json, PROJECT-RULES.json, architecture map, and epic input. Write REQUIREMENT.md with scope, non-goals, numbered acceptance criteria, and observable behavior. Do not mutate production code.',
...
export function generatedCofofoWorkspace(current?: Partial<WorkspaceConfig>): WorkspaceConfig {
  ...
  const delivery = {
    ...
    steps: [
      step('requirement', { requires: [`${FOUNDATION}/CONTEXT-MANIFEST.json`], produces: [`${EPIC}/REQUIREMENT.md`], produces_contains: ['## Acceptance Criteria'] }),
