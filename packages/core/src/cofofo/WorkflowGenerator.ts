...
const PHASE_INSTRUCTIONS: Record<Phase, string> = {
  ...
  requirement: 'Read the current CONTEXT-MANIFEST.json, PROJECT-RULES.json, architecture map, and epic input. Write REQUIREMENT.md with scope, non-goals, numbered acceptance criteria, and observable behavior. The Canvas gate on REQUIREMENT.md must approve scope before diagnose or planning. Do not mutate production code.',
...
export function generatedCofofoWorkspace(current?: Partial<WorkspaceConfig>): WorkspaceConfig {
  ...
  const delivery = {
    ...
    steps: [
      step('requirement', { requires: [`${FOUNDATION}/CONTEXT-MANIFEST.json`], produces: [`${EPIC}/REQUIREMENT.md`], produces_contains: ['## Acceptance Criteria'], human_review: true, review: { mode: 'canvas', artifacts: [`${EPIC}/REQUIREMENT.md`] } }),
