...
describe('built-in SDLC workflow packs', () => {
  it('ships four versioned packs with five-stage guide metadata', () => {
    expect(listBuiltinWorkflowPacks().map((pack) => pack.id)).toEqual(['sdlc-core', 'speckit', 'cohesive', 'regulated']);
  ...
  it('maps speckit, cohesive parallel work and regulated traceability explicitly', () => {
    expect(resolveBuiltinWorkflowPack('speckit').actions?.understand?.[0].id).toBe('speckit-specify');
    expect(resolveBuiltinWorkflowPack('cohesive').actions?.build?.filter((item) => item.subrun)).toHaveLength(2);
