...
describe('Project Workspace companion command files', () => {
  const tempRoots: string[] = [];
  afterEach(() => {
    for (const root of tempRoots.splice(0)) fs.rmSync(root, { recursive: true, force: true });
  });

  it('expects 6 distinct command ids for the three Project Workspace pipelines', () => {
  ...
  it('loadBuiltinPreset has skill content for every Project Workspace phase', () => {
    const workflow = BUILTIN_WORKFLOWS.find((w) => w.id === 'project-workspace')!;
