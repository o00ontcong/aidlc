...
const workflow = BUILTIN_WORKFLOWS.find((entry) => entry.id === 'project-workspace')!;
...
describe('Project Workspace built-in bundle', () => {
  it('materializes three pipelines: project-context, feature-spike, feature-implement', () => {
    ...
    const config = validateWorkspace(
      { name: 'project-workspace-test', ...preset.workspace },
      'project-workspace/workspace.yaml',
    );
  ...
  it('lets a human list and resolve pending validator conflicts without manual file surgery', () => {
    ...
    const bundledShip = fs.readFileSync(path.join(ROOT, 'templates', 'project-workspace', 'validators', 'ship.mjs'), 'utf8');
