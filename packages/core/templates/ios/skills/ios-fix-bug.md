...
|---|---|---|
| Thêm bản ghi mà danh sách không đổi | state không phát thay đổi (`@Published`/`@Observable`), hoặc mutate ngoài store | View layout |
| Thứ tự hiển thị sai | computed property lọc + sắp xếp | thứ tự trong mảng gốc |
| Dữ liệu mất sau khi thao tác | thiếu bước persist sau mutation | tầng persistence |
| Validate không chạy | luật bị đặt nhầm trong View | tầng sở hữu state |
