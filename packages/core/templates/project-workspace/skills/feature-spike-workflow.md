...
## Graph completeness (Flow / Surfaces / Impact — required)

These three graphs are the same class of artifact as project-context catalogs: inventory the epic path, then close over edges. A two-node happy path is invalid when AC/Tasks or SCREEN-CATALOG show more destinations.

1. **Inventory sources this epic changes** — task files, coordinators/routers, overlay/step enums, catalog nodes whose evidence files you already cite. Use ast-graph when available.
2. **One node per real unit on this path.** Expand Layer 1/2, MFA method pickers, and password/step machines. Do not collapse an overlay host into one box.
3. **One edge per distinct trigger** with evidence. Same pair, different guard → another edge.
4. **`discovery` on each JSON** (`method`, `sources[]`, `unknowns[]`). Empty `unknowns` only when every cited screen/feature is on the graph.
5. If `SCREEN-CATALOG.json` / `FEATURE-CATALOG.json` exist, every catalog entry evidenced by a file this pack already lists must appear on FEATURE-FLOW / FEATURE-IMPACT or in `unknowns`. Do not skip because a prior pack existed.
...
- **Flow** — mermaid fence of the user/API path through the code that will change. Copy that **same** mermaid into `FEATURE-FLOW.mmd`. Also write `FEATURE-FLOW.json` (`schemaVersion: 1`, `featureId`, nodes with `id`/`label`/`file`, edges, `discovery`). Do not invent a different flow in JSON. Do not stop at a two-step sketch.

Also write (required, not optional):

- `FEATURE-SURFACES.json` + `FEATURE-SURFACES.mmd` — web/mobile/desktop/api/worker/sdk/external nodes; edges `http|sdk|event|webhook|internal`; `discovery`. External nodes must not invent a workspace `file`. Include every system this epic actually talks to.
- `FEATURE-IMPACT.json` + `FEATURE-IMPACT.mmd` — vs `docs/project/context/visualization/FEATURE-CATALOG.json`. Each feature `change`: `add|modify|delete|unchanged`. At least one add/modify/delete. `modify`/`delete` ids must exist in the catalog; `add` ids must not. Catalog features evidenced by this epic's files must be listed (use `unchanged` if needed) or named in `discovery.unknowns`.
