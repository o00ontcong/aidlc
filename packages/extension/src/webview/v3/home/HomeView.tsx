...
export function HomeView({ state, client, onOpenEpics, onOpenStudio }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {state.cohesiveUpgrade && <section className="rounded-md border border-sky-500/50 bg-sky-500/5 p-4"><h2 className="text-sm font-semibold text-foreground">Feature-centric Cohesive upgrade available</h2><p className="mt-1 text-xs text-muted-foreground">This project uses a previous Cohesive Delivery workflow. Upgrade adds Architecture → Feature → Code exploration and preserves active runs with pipeline snapshots.{state.cohesiveUpgrade.activeRunCount ? ` ${state.cohesiveUpgrade.activeRunCount} active run(s) will be protected.` : ''}</p>{state.cohesiveUpgrade.hasConflicts && <p className="mt-2 text-xs text-amber-600">Custom pipeline phases were detected; AIDLC will show the conflict before changing anything.</p>}<button type="button" onClick={() => client.dispatch(command('cohesive.upgrade.open', {}))} className="mt-3 rounded bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90">Review &amp; upgrade</button></section>}
