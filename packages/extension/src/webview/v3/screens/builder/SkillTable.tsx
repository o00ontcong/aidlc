...
import { Card, mock } from '../../components';
import { MOCK_SKILLS } from '../../data/mock-data';
import type { SkillVM } from '../../data/types';

const SOURCE_CLS: Record<SkillVM['source'], string> = {
  ...
  return (
    ...
    >
      {MOCK_SKILLS.map((skill, i) => (
        <div
          key={skill.id}
          className={`flex items-center p-[9px_14px] gap-[11px] ${i < MOCK_SKILLS.length - 1 ? 'border-b border-bd2' : ''}`}
        >
          ...
          <button type="button" onClick={() => {}} className="flex-none text-[11.5px] text-txt2">Edit</button>
          <button type="button" onClick={() => {}} className="flex-none text-[11.5px] text-err">Delete</button>
        </div>
      ))}
    </Card>
  );
