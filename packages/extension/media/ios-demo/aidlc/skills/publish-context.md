...
description: Xuất bản nền — chỉ mục docs/, khối chỉ dẫn AI trong CLAUDE.md, và CONTEXT-MANIFEST.json mở khoá ios-pipeline.
---
...
`ios-pipeline` khai `requires` manifest này — **chưa publish thì không epic nào đóng được bước `requirement`**.
...
1. **`docs/README.md`** — `## Reading Order` (bảng có thứ tự: đọc file nào, trả lời câu hỏi gì) và `## Map` (mỗi thư mục con chứa gì, bước nào ghi ra).
2. **Khối `aidlc:context` trong `CLAUDE.md`** — một câu hệ thống là gì · 4 đường dẫn phải đọc · `INV-n` một dòng mỗi cái · hai pipeline và trigger · câu chốt "chi tiết ở docs/README.md".
